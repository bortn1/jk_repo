...
public class MainActivityATest {
    ...
    @Test
    public void WhenUserOpenApplicationListOfRepositoryLoadedToTheScreen() {
        verifyThatTextDisplayedOnTheScreen(TestConsts.FIRST_VISIBLE_ITEM, FIRST_VISIBLE_ITEM);
    }

    @Test
    public void ThenUserScrollDownAndMoreDataLoadedWithPagination() {
    ...
    @Test
    public void AndUserScrollsToTheBottomOfThePageAndVerifyThatAllDataLoadedAndNoMoreBeingLoaded() {
        for (int i = 0; i < 7; i++) {
            onView(withId(R.id.recycler_view))
